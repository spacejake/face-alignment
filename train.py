...
from face_alignment.util.evaluation import AverageMeter, calc_metrics, accuracy_points, calc_nme, get_preds
    ...
    if args.resume and train_fan:
        if os.path.isfile(args.resume):
            ...
            if 'state_dict' in checkpoint:
                ...
                args.start_epoch = 0
                best_acc = 0
                model.FAN.load_state_dict(checkpoint)

            print("=> Loaded FAN checkpoint '{}' (epoch {})".format(args.resume, args.start_epoch))
...
def validate(loader, model, criterion, netType, debug, flip, device):
    ...
    for val_idx, (inputs, label, meta) in enumerate(loader):
        roi_boxes = meta['roi'] if 'roi' in meta else None
        target = Target._make(label)
        data_time.update(time.time() - end)
    ...
    # print("=> Mean Error: {:.2f}, AUC@0.07: {} based on maps".format(mean_error, auc))
